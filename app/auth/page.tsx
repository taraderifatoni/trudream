...
async function apiSignIn(email: string, password: string) {
  ...
  if (d.access_token) {
    ...
    const longExp    = new Date(Date.now() + 30 * 24 * 3600 * 1000).toUTCString() // 30 hari
    document.cookie = `sb-access-token=${d.access_token}; expires=${accessExp}; path=/; SameSite=Lax`
    document.cookie = `sb-refresh-token=${d.refresh_token}; expires=${longExp}; path=/; SameSite=Lax`
    document.cookie = `sb-user=${encodeURIComponent(JSON.stringify({ email: d.user?.email || email }))}; expires=${longExp}; path=/; SameSite=Lax`
  }
  return d
}
...
function AuthForm() {
  ...
  return (
    <div style={{ background: D, minHeight: '100vh', color: W, fontFamily: "'IBM Plex Mono','Courier New',monospace", position:'relative', zIndex:2 }}>
      ...
      <header style={{ position:'sticky',top:0,zIndex:20,display:'flex',alignItems:'center',justifyContent:'space-between',padding:'12px 16px',background:'rgba(25,30,41,0.94)',backdropFilter:'blur(12px)',borderBottom:'1px solid rgba(255,255,255,0.08)',maxWidth:760,margin:'0 auto' }}>
      ...
      <main style={{display:'flex',alignItems:'center',justifyContent:'center',padding:'60px 16px'}}>
        <div style={{background:S,border:'1px solid rgba(255,255,255,0.08)',borderRadius:16,padding:'32px 28px',width:'100%',maxWidth:400,display:'flex',flexDirection:'column',gap:22}}>
          ...
          <form onSubmit={handleSubmit} style={{display:'flex',flexDirection:'column',gap:14}}>
            ...
            <button type="submit" disabled={loading} className="pixel abtn" style={{minHeight:46,background:L,color:'#191E29',border:`2px solid ${L}`,borderRadius:12,fontSize:11,cursor:loading?'not-allowed':'pointer',boxShadow:'4px 4px 0 rgba(1,195,141,0.3)',opacity:loading?0.6:1}}>
              {loading ? 'WAIT...' : mode === 'login' ? 'Masuk' : 'Daftar'}
            </button>
          </form>
          ...
          {error ? <div style={{fontSize:12,color:error.startsWith('✓')?L:'#ef4444',textAlign:'center',padding:'8px',background:'rgba(255,255,255,0.03)',borderRadius:8}}>{error}</div> : null}
