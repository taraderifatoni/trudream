  ...
  try {
    ...
    for (const sel of containers) {
      ...
      $(sel).find('img').each((_, el) => {
        ...
        assets.push({ type: 'image', url: fullUrl, caption: alt, source: 'article-img', priority: priority++, width: parseInt($(el).attr('width') || '0') || undefined, height: parseInt($(el).attr('height') || '0') || undefined })
        found++
      })
